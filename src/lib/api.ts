...
export const adminApi = {
  getAllUsers: async (page = 1, limit = 10, searchTerm = ''): Promise<{ data: User[], totalPages: number, total: number }> => {
    const response = await api.get('/admin/users', { params: { page, limit, searchTerm } });
    return response.data;
  },
